...
#' @param save_dir Directory to save plots if save_plots=TRUE (default: "ModelData/clinical_predictions")
...
#' @param plot_display_num Number of plots to display if save_plots=FALSE (default: 1)
...
#' @param save_data Logical indicating whether to save processed data (default: TRUE)
#' @param csv_filename Filename for saving processed data if save_data=TRUE (default: "new_data.csv")
#'
...
process_new_data <- function(
  ...
  group_col_in_object <- object@group_col
  if (group_col_in_object %in% colnames(new_data)) {
  ...
  cleaned_test<-convert_factors_to_binary(cleaned_test)
  factor_vars <- sapply(object@filtered.set$training, is.factor)
  for (var in names(factor_vars)[factor_vars]) {
    if (var %in% colnames(cleaned_test)) {
      if (is.factor(object@filtered.set$training[[var]])) {
        cleaned_test[[var]] <- factor(cleaned_test[[var]],
                                      levels = levels(object@filtered.set$training[[var]]))
      } else {
        cleaned_test[[var]] <- as(cleaned_test[[var]], class(object@filtered.set$training[[var]]))
      }
    }
  }
  if (data_type == "scale") {
